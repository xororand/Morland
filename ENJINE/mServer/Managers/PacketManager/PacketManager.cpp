...
void PacketManager::c_register_user(Peer* peer, sf::Uint8 status) {
	enjPacket p;
	p << (sf::Uint16)C_REGISTER_USER << status; // Îòïðàâëÿåì ñîîáùåíèå îá ñîñòîÿíèè ðåãèñòðàöèè
	send_packet(peer, p);
}
...
void PacketManager::c_login_user(Peer* peer, sf::Uint8 status) {
	enjPacket p;
	p << (sf::Uint16)C_LOGIN_USER << status; // Îòïðàâëÿåì ñîîáùåíèå îá ñîñòîÿíèè àâòîðèçàöèè
	send_packet(peer, p);
}
